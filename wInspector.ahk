...
Gui_wInspector(*){
    ...
    ogBut_MouseMove.OnEvent("Click", (*) => (CoordMode("Mouse", ogDDL_MouseCoordMode.Text), MouseMove(oGuiMouse.MouseX+0,oGuiMouse.MouseY+0)))
    ogBut_MouseClick := oGuiMouse.AddButton("x+3 yp w50", "Click")
    ogBut_MouseClick.OnEvent("Click", (*) => (CoordMode("Mouse", ogDDL_MouseCoordMode.Text), Mouseclick(,oGuiMouse.MouseX+0,oGuiMouse.MouseY+0)))
...
GuiRectangle(x:= 0, y:= 0 ,w:= 100 ,h:=100 , Color:="Blue",Thickness := 2){
    ...
    MovePos(GuiBox, x:="", y:="", w:="", h:="",Thickness:=""){
        ...
        GuiBox.Move(x / ScreenScale, y / ScreenScale, w / ScreenScale, h / ScreenScale)
        goColor.Move(, , w / ScreenScale, h / ScreenScale)
        goTransp.Move(Thickness, Thickness, w / ScreenScale - Thickness * 2, h / ScreenScale - Thickness * 2)
        ; GuiBox.Move(x, y, w, h)
        ; goColor.Move(,,w,h)
...
Gui_About(){
    ...
    ogAbout.Add("Text", "x53 +0x80 +E0x4", "Written by Ahk_user.`n© All rights reserved.")
...
GuiAccViewer(Wintitle:="A"){
    ...
    LVAcc := myAccGui.Add("ListView", "xm yp+24 r25 w800", ["Path","Name","RoleText","Role","x","y","w","h","Value", "StateText", "State", "Description", "KeyboardShortcut", "Help", "ChildId"])
